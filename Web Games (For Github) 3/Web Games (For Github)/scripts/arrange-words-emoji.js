const correctPairs = {
    ...
    'item2Bottom': 'item3Top',
    'item3Bottom': 'item1Top'
}
...
document.getElementById('checkBtn').addEventListener('click', function() {
    ...
    Object.keys(correctPairs).forEach(textClass => {
        ...
        if (!textEl || textEl.dataset.stuckTo !== correctImageClass) {
            allCorrect = false
        }
    })
    ...
    if (allCorrect) {
        ...
        texts.forEach(textEl => {
            if (textEl && textEl.dataset.stuckTo) {
...
document.getElementById('next').addEventListener('click', function() {
    window.location.href = 'arrange-words-level-1.html'
})
